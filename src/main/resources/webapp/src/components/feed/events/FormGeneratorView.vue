            ...
            <div v-for="(field, index) in formInputs" :key="index" class="col s12">
                <template v-if="['text', 'number'].indexOf(field.type) > -1">
                    <div class="input-field col s6 offset-s3">
                        <input :disabled="isPreview" :id="`form-field-${index}`" type="text"
                               v-model="userFormInput[field.label]">
                ...
                <template v-else-if="field.type === 'select'">
                    <div class="input-field col s6 offset-s3">
                        <select :disabled="isPreview" id="main-select" v-model="userFormInput[field.label]">
                            <option v-for="(formOption, index) in field.options.split(', ')" :key="index"
                ...
                <template v-else-if="field.type === 'image'">
                    <form class="col s6 offset-s3">
                        <div class="file-field input-field">
                            <div class="btn green">
                                <span>File</span>
                                <input :disabled="isPreview" type="file" @change="processFile($event, userFormInput, field.label)">
                            </div>
